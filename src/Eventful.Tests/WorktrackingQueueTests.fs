...
module WorktrackingQueueTests = 
    ...
    let ``Completion function is called when item complete`` () : unit =
        ...
        let worktrackingQueue = new WorktrackingQueue<string,(string * string)>(1000, 100000, groupingFunction, complete, 10, fun _ _ -> Async.Sleep(100)  )
    ...
    let ``Given item split into 2 groups When complete Then Completion function is only called once`` () : unit =
        ...
        async {
            let worktrackingQueue = new WorktrackingQueue<int,(string * string)>(1000, 100000, groupingFunction, complete, 10, fun _ _ -> Async.Sleep(100)  )
    ...
    let ``Given empty queue When complete Then returns immediately`` () : unit =
        let worktrackingQueue = new WorktrackingQueue<unit,string>(1000, 100000, (fun _ -> Set.singleton ()), (fun _ -> Async.Sleep(1)), 10, (fun _ _ -> Async.Sleep(1)))
